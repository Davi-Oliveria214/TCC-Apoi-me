...
<div class="modal-overlay" style="display:flex;">
   ...
   ===================================================================== */
    elseif ($tipo === 'avaliar'):
        ...
        <?php else:
            ...
            $status  = $statusMap[$c['confirmado']] ?? ucfirst($c['confirmado'] ?? 'Pendente');
        ?>
   ...
   ===================================================================== */
    elseif ($tipo === 'ver_avaliacao'):
        $avaliacao = request("avaliacoes?id=eq.$id&select=*,contratados(dia,hora)");
        ...
        <?php else:
            $av = $avaliacao[0];
            $dataBruta = !empty($av['data']) ? $av['data'] : ($av['contratados']['dia'] ?? '');
            $horaBruta = !empty($av['horario']) ? $av['horario'] : ($av['contratados']['hora'] ?? '');

            $dataFmt = !empty($dataBruta) ? date('d/m/Y', strtotime($dataBruta)) : '—';
            $horaFmt = !empty($horaBruta) ? date('H:i', strtotime($horaBruta)) : '—';
        ?>
                    ...
                    <div class="detalhes-lista">
                        ...
                        <div class="input-row">
                            <div class="detalhe-item">
                                ...
                                <p>R$ <?= number_format($c['preco_contrato'] ?? 0, 2, ',', '.') ?> / <?= esc($c['servicos']['tipo_cobrado'] ?? 'serviço') ?></p>
                            </div>
                        </div>
                    </div>
                </div>
